...
router.get("/product/:id", async (req, res, next) => {
  try {
    ...
    const [product] = await db.query(
      `SELECT id, ean, name, brand, category, image_url, spec_json,
              first_seen_source, source_id, external_id,
    ...
    // Current prices from all retailers (include previous_price and via_source)
    const prices = await db.query(
      `SELECT retailer, price_sek, previous_price, in_stock, affiliate_url,
              via_source, scraped_at
       FROM dsc_prices
       WHERE product_id = ?
    ...
    // Product sources matched to this product — look up by BOTH ean and product_id
    // This ensures we find rows regardless of whether matching was by EAN or external_id
    const sources = await db.query(
      `SELECT ps.external_id, ps.ean, ps.match_status, ps.scraped_at,
              ps.source_id
       FROM dsc_product_sources ps
       WHERE ps.matched_product_id = ?
          OR (ps.ean IS NOT NULL AND ps.ean = ?)
