...
function mapAwinRow(row, retailer) {
  const price = parseFloat(
    row["Selling Price"] || row["Price"] || row["price"] || "0",
  );
  const ean = (row["EAN"] || row["GTIN"] || row["ean"] || "").replace(
    /\s/g,
    "",
  );
  ...
  for (const feedUrl of feedUrls) {
    ...
    try {
      ...
      const records = takeRemaining(
        rows.map((row) => mapAwinRow(row, retailer)).filter((r) => r.ean),
        allRecords.length,
        itemLimit,
      ); // Only include records with a valid EAN candidate

      log.info(`Awin feed parsed`, {
        retailer,
      ...
      log.error("Failed to download/parse Awin feed", {
        feedUrl,
        err: err.message,
      });
    }
