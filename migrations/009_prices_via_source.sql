...
SET @add_col = (
  SELECT IF(
    ...
    'SELECT 1',
    'ALTER TABLE dsc_prices ADD COLUMN via_source VARCHAR(64) DEFAULT NULL AFTER affiliate_url'
  )
);
...
SET @backfill = (
  SELECT IF(
    ...
    'UPDATE dsc_prices SET via_source = ''prisjakt'' WHERE retailer = ''prisjakt'' AND via_source IS NULL',
    'SELECT 1'
  )
);
