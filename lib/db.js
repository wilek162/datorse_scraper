...
async function query(sql, params) {
  // Use .execute() (prepared statement) only when params are present.
  // Passing empty/undefined params to execute() fails in some mysql2 versions.
...
async function upsertProduct(record, sourceId) {
  ...
  try {
    ...
    // 4. Check whether the existing row is owned by a dedicated scraper.
    // A dedicated-scraper row (via_source != 'prisjakt') must not be overwritten
    // by Prisjakt-sourced data. A Prisjakt row can always be upgraded.
    const incomingViaSource = record.via_source ?? sourceId;
    const [viaRows] = await conn.execute(
      `SELECT via_source FROM dsc_prices WHERE product_id = ? AND retailer = ?`,
      [productId, record.retailer],
    );
    const existingViaSource = viaRows[0]?.via_source ?? null;
    const isProtected =
      existingViaSource !== null &&
      existingViaSource !== "prisjakt" &&
      incomingViaSource === "prisjakt";
    ...
    // 5. Upsert current price (with via_source)
    await conn.execute(
      `INSERT INTO dsc_prices
         (product_id, retailer, price_sek, previous_price, in_stock, affiliate_url, via_source, scraped_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
    ...
    // 6. Append to price history only on price or availability change
    if (
      lastPrice === null ||
...
async function upsertOfferPrice(productId, record, sourceId) {
  ...
  try {
    ...
    // Respect via_source priority: don't overwrite dedicated-scraper rows
    // with Prisjakt-sourced data.
    const incomingViaSource = record.via_source ?? sourceId;
    const [viaRows] = await conn.execute(
      `SELECT via_source FROM dsc_prices WHERE product_id = ? AND retailer = ?`,
      [productId, record.retailer],
    ...
    if (isProtected) {
      await conn.rollback();
      return { upserted: false, suspicious: false, skippedProtected: true };
    }
...
async function markProductSourceMatched(
  ...
  if (ean) {
    ...
    // No LIMIT 1 here: multiple offer records from the same product share the
    // same external_id (e.g. Prisjakt merchant rows), all should be marked matched.
    await getPool().execute(
      `UPDATE dsc_product_sources
       SET match_status = 'matched', matched_product_id = ?
...
async function isSourcePaused(sourceId) {
  const rows = await query(
    `SELECT paused FROM dsc_source_flags WHERE source_id = ? LIMIT 1`,
    [sourceId],
  );
  return rows.length > 0 && rows[0].paused === 1;
}

async function setSourcePaused(sourceId, paused, pausedBy = null, note = null) {
  await getPool().execute(
    `INSERT INTO dsc_source_flags (source_id, paused, paused_by, note)
     VALUES (?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE
    ...
    [sourceId, paused ? 1 : 0, pausedBy, note],
  );
}
