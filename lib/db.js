...
async function finishScrapeLog(logId, { recordsFound, recordsValid, recordsUpserted, status, errorMessage }) {
  await getPool().execute(
    `UPDATE dsc_scrape_log
     SET finished_at      = NOW(),
         ...
         error_message    = ?
     WHERE id = ?`,
    [recordsFound, recordsValid, recordsUpserted, status, errorMessage ?? null, logId],
  );
}
