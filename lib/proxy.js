...
const _rateLimitQueue = new Map();
const _nextAllowedAt = new Map();
...
function getRateLimitInterval(sourceConfig = {}) {
  const reqPerMin = Number(sourceConfig.rateLimit?.reqPerMin || 0);
  if (!Number.isFinite(reqPerMin) || reqPerMin <= 0) return null;
  return Math.ceil(60_000 / reqPerMin);
}

async function waitForRateLimit(sourceConfig, log) {
  const intervalMs = getRateLimitInterval(sourceConfig);
  const sourceId = sourceConfig.id;

  if (!intervalMs || !sourceId) return;

  const queue = _rateLimitQueue.get(sourceId) || Promise.resolve();
  const nextStep = queue.then(async () => {
    const now = Date.now();
    const nextAllowed = _nextAllowedAt.get(sourceId) || now;
    const waitMs = Math.max(nextAllowed - now, 0);

    if (waitMs > 0) {
      log.debug("Rate limit pause", { sourceId, waitMs });
      await sleep(waitMs);
    }

    _nextAllowedAt.set(sourceId, Date.now() + intervalMs);
  });
...
// ─── Account-level fatal error ────────────────────────────────────────────────
/**
 * Thrown when the proxy service itself is unavailable account-wide
 * (suspended account, monthly limit exceeded, etc.) rather than a
 * per-URL permanent error. Scrapers should re-throw this so the
 * runner records a "failed" status with the actual reason instead of
 * the generic "No records fetched" partial.
 */
class ProxyFatalError extends Error {
  constructor(message) {
    super(message);
    this.name = "ProxyFatalError";
    this.isFatal = true;
  }
}

/**
 * Extracts a human-readable message from a Zyte API error response.
 * Returns a ProxyFatalError for account-level issues, enriched Error otherwise.
 */
function _enrichZyteError(axiosErr) {
  const data = axiosErr.response?.data;
  if (!data) return axiosErr;

  const title = data.title || "";
  const detail = data.detail || title;
  const type = data.type || "";
  const status = axiosErr.response?.status;
  const message = `Zyte ${title} (${status}): ${detail}`;

  if (
    type.includes("account-suspended") ||
    type.includes("payment") ||
    type.includes("billing")
  ) {
    return new ProxyFatalError(message);
  }
...
/**
 * Extracts a human-readable message from a Scrape.do API error response.
 * Returns a ProxyFatalError for account-level issues, enriched Error otherwise.
 */
function _enrichScrapeDoError(axiosErr) {
  const data = axiosErr.response?.data;
  if (!data) return axiosErr;
  ...
  const messages = Array.isArray(data.Message)
    ...
    : (data.Message || "");
  const status = axiosErr.response?.status;
  const message = `Scrape.do error (${status}): ${messages || JSON.stringify(data)}`;

  if (
    messages.toLowerCase().includes("limit exceeded") ||
    messages.toLowerCase().includes("suspended") ||
    messages.toLowerCase().includes("upgrade your plan")
  ) {
    return new ProxyFatalError(message);
  }
...
async function _fetchViaScrapeDo(url, renderJs, log, sourceConfig) {
  ...
  return pRetry(
    async (attempt) => {
      try {
        await waitForRateLimit(sourceConfig, log);
        ...
        const enriched = _enrichScrapeDoError(axiosErr);
        if (enriched instanceof ProxyFatalError)
          throw new pRetry.AbortError(enriched);
        const status = axiosErr.response?.status;
  ...
  return pRetry(
    async (attempt) => {
      try {
        ...
        if (axiosErr instanceof pRetry.AbortError) throw axiosErr;
        const enriched = _enrichZyteError(axiosErr);
        if (enriched instanceof ProxyFatalError)
          throw new pRetry.AbortError(enriched);
        const status = axiosErr.response?.status;
        if (isPermanentStatus(status)) throw new pRetry.AbortError(enriched);
        throw enriched;
      }
    },
    {
