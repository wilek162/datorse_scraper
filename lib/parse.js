 ...
 * @param {CheerioAPI} $
 * @returns {object[]} Flat array of all JSON-LD objects found (arrays are spread).
 */
...
/**
 * Extracts the Next.js __NEXT_DATA__ object from a Cheerio-loaded page.
 * Returns the parsed object, or null if not present / malformed.
 * @param {CheerioAPI} $
 * @returns {object|null}
 */
...
/**
 * Recursively searches a plain object/array for an array matching `predicate`.
 * Returns the first matching array found, or null.
 * Useful for fishing out product arrays from __NEXT_DATA__ trees.
 *
 * @param {*}        node       - Any value to search
 * @param {Function} predicate  - (array) => boolean — called on every array found
 * @param {number}   [maxDepth] - Stop recursion beyond this depth (default 10)
 * @returns {Array|null}
 */
function findArrayInObject(node, predicate, maxDepth = 10) {
  if (
    maxDepth <= 0 ||
    node === null ||
    node === undefined ||
  ...
  if (Array.isArray(node)) {
    ...
    for (const item of node) {
      const found = findArrayInObject(item, predicate, maxDepth - 1);
      if (found) return found;
    }
    return null;
  }
  for (const val of Object.values(node)) {
    const found = findArrayInObject(val, predicate, maxDepth - 1);
    if (found) return found;
  }
  return null;
}
